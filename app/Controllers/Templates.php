	...
	public function AddTemplate()
	{
		...
		if ($this->request->getMethod() == 'post') {
			...
			if ($result['code'] == 1) {
				...
				$contact_usModel = new Contact_usModel();
				$black_list = $contact_usModel->get_blacklist();
			  
				if(!empty($black_list)){
					
					$black_list = str_replace(" ", "|", $black_list->text);
					$black_list = str_replace("||", "|", $black_list);
					$black_list = str_replace("||", "|", $black_list);
					if ($black_list[strlen($black_list)-1] == "|") {
						$black_list = substr($black_list, 0, strlen($black_list)-1); 
					}

					
					if (preg_match('/('.$black_list.')/i', $content)) {
						$result=array('code'=>-1,'msg'=>'نص القالب يحتوي كلمات مسيئة '
					);
