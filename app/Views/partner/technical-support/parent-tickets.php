                    ...
                    <div class="card-body">
                        <div class="row ">
                            ...
                            <div class="d-flex col-sm-3">
                                <button type="button" id="close-ticket-btn" style="width: inherit; padding: .375rem .75rem;" class="btn btn-primary" onclick="closeTicket()">
                                    اغلاق التذكرة
                                </button>
                            </div>
                        </div>
...
<script>
    ...
    function showReplyModal(id, ticket_text, date, username, department, type, status, prority) {
        ...
        if (status == '1') {
            $("#close-ticket-btn").attr('disabled', 'true');
            $('#modal-status').html('مغلقة');
        } else {
            $("#close-ticket-btn").removeAttr('disabled');
            $('#modal-status').html("مفتوحة");
        }
    ...
    function closeTicket() {
        ...
        var jqxhr = $.ajax({
            ...
            .done(function(response) {
                $("#close-ticket-btn").attr('disabled', 'true');
                toastr.success("تم تحديث حالة البطاقة الى <مغلقة>");
            })
            .fail(function(response) {
                console.log(response);
