...
<div class="modal fade" id="update-period" tabindex="-1" aria-labelledby="update-periodLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="update-periodLabel">تحديث معلومات حصة</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<div class="modal fade" id="add-semester" tabindex="-1" aria-labelledby="add-semesterLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="add-semesterLabel">اضافة فصل</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form onsubmit="addSemester(this); return false;">

                <div class="modal-body">
                    <div class="row">
                        <div class="col-md">
                            <div class="form-group">
                                <label for="add-semester-name">العنوان</label>
                                <input required type="text" name="name" class="form-control" id="add-semester-name">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" style="margin-left: auto;" data-dismiss="modal">الغاء</button>
                    <button type="reset" class="btn btn-secondary">مسح</button>
                    <button type="submit" id="add-semester-submit" class="btn btn-primary">حفظ</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="update-semester" tabindex="-1" aria-labelledby="update-semesterLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="update-semesterLabel">تحديث فصل</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form onsubmit="updateSemester(this); return false;">
                <input type="hidden" name="id" id="update-semester-id">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md">
                            <div class="form-group">
                                <label for="update-semester-name">العنوان</label>
                                <input required type="text" name="name" class="form-control" id="update-semester-name">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" style="margin-left: auto;" data-dismiss="modal">الغاء</button>
                    <button type="reset" class="btn btn-secondary">مسح</button>
                    <button type="submit" id="update-semester-submit" class="btn btn-primary">حفظ</button>
                </div>
            </form>
        </div>
    </div>
...
<!-- Button trigger modal -->
<button type="button" style="display: none;" id="update-semester-btn" class="btn btn-primary" data-toggle="modal" data-target="#update-semester"></button>
...
<div class="row">
    ...
    <div class="col-md-5">
        <div class="card">
            ...
            <div class="card-header  p-2 d-flex align-items-center justify-content-end bg-white">
                <div style="margin-left: auto;">
                    <h6 class=" text-bold">الفصول</h6>
                </div>
                <div class="mx-1">
                    <button type="button" class="btn btn-light" data-toggle="modal" data-target="#add-semester">اضافة فصل</button>
                </div>
            </div>
            <!-- start card body1 -->
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12" style="overflow-x: scroll;">
                        ...
                        <table id="semesters-table" class="table table-striped " style="width:100% ">
                            <thead>
                                <tr>
                                    <th>م</th>
                                    <th>الاسم</th>
...
<script>
    ...
    $(document).ready(function() {
        semestersDataTable = $('#semesters-table').DataTable({
            dom: `<"row d-flex justify-content-end mx-1 my-1 mb-3 "B><"row d-flex justify-content-between mx-1 "fl>rtip`,
            "lengthMenu": [
                [25, 50, 100, 500],
                [25, 50, 100, 500]
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        return `<div class="row justify-content-center  text-white ">
                                    <div class="m-1">
                                        <button type="button" class="btn btn-sm btn-info " title="edit" style="margin: 0px;" onclick="showUpdateSemesterModal('${row.id}', '${row.name}')" class="nav-link">
                                            <i class="far fa-edit"></i>
                                        </button>
                                    </div>

                                    <div class="m-1">
                                        <a type="button" class="btn btn-sm btn-danger " title="delete" style="margin: 0px;" onclick="DeleteSemester('${row.id}')">
                                            <i class="far fa-trash-alt"></i>
                                        </a>
                                    </div>
                                </div>`;
    ...
    function semesters() {
        var jqxhr = $.ajax({
            ...
            .done(function(response) {
                semestersDataTable.clear().rows.add(response.data).draw()
            })
            .fail(function(response) {
                console.log(response);
    ...
    function addSemester(element) {
        ...
        $.ajax({
            url: "<?= site_url('') ?>schools/addSemester",
            method: "POST",
            "headers": {
                "Authorization": token
            },
    ...
    function showUpdateSemesterModal(id, name) {

        $("#update-semester-id").val(id);
        $("#update-semester-name").val(name);

        $("#update-semester-btn").click();
    }
    ...
    function updateSemester(element) {
        ...
        $.ajax({
            url: "<?= site_url('') ?>schools/updateSemester",
            method: "POST",
            "headers": {
                "Authorization": token
            },
    ...
    function DeleteSemester(id) {
        if (confirm("هل انت متأكد من حذف السجل؟!"))
            var jqxhr = $.ajax({
                    url: "<?= site_url('') ?>schools/deleteSemester",
                    method: "DELETE",
                    timeout: 0,
                    data: {
                        id: id,
